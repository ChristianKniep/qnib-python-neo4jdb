...
class SlurmJob(object):
    ...
    def render(self):
        ...
        if 'started' not in self.job.keys() or not isinstance(self.job['started'], int):
            if 'start' in self.job.keys():
                dt_start = datetime.fromtimestamp(int(self.job['start']))
                self.job['started'] = dt_start.strftime("%Y-%m-%dT%H:%M:%S")
            else:
...
class N4jDB(GraphDB):
    ...
    def merge_hca_link(self, link, seen):
        ...
        dst = self.sw_by_guid({'src_node_guid': link['dst_guid']})
        link['dst_name'] = dst['src_node_name']
        name = "%s[%s]->[%s]%s" % (
            link['src_name'].split("_")[0], link['src_port'],
            link['dst_port'], link['dst_name'],
        )
    ...
    def merge_sw_link(self, link, seen):
        ...
        name = "%s[%s]->[%s]%s" % (
            link['src_name'], link['src_port'],
            link['dst_port'], link['dst_name'].split("_")[0],
        )
        query = "MATCH (src)<-[:PART_OF]-(sp:PORT {guid:'%(src_guid)s', port_int:'%(src_port)s'})," % link
