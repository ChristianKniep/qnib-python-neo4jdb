...
class SlurmJob(object):
    ...
    def render_running(self):
        ...
        return  jobid, jobname, user, start, nodes, duration
    ...
    def render_finished(self):
        ...
        #  jobid, jobname, user, start, state, nodes, duration, ec
        """
        jobid = self.job['id']
        jobname = self.job['name']
        user = self.job['user']
        ...
        ec = "?"
        return  jobid, jobname, user, start, state, nodes, duration, ec
...
class N4jDB(GraphDB):
    ...
    def merge_hca_link(self, link, seen):
        ...
        query += " MERGE (src)-[:LINKED]->(c0)<-[:LINKED]-(dest)"
    ...
    def fetch_topology(self):
        ...
        """
        query = "MATCH (s)-[:LINKED]->(c:Connection)<-[:LINKED]-(d) RETURN s, d"
        res = self._gdb.query(query, data_contents=True)
        topo_cache = set([])
        topo = []
        for row in res.graph:
